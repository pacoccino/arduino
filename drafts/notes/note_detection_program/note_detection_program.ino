...
{ long unsigned int a1, b, a2;
  ...
  for (int i = 0; i < 128; i++)
  {
    a = analogRead(Mic_pin); //rough zero shift
    ...
    a = a * (sin((i/255) * 3.14 / 2) * sin(i * 3.14 / 128)); // Hann window
  ...
  //Serial.println(String(sum2));
  //Serial.println(String(sum1));
  ...
  if (sum2 - sum1 > 2) {
    ...
    Serial.print(f_peaks[0]); Serial.print(" | ");
    Serial.print(f_peaks[1]); Serial.print(" | ");
    Serial.print(f_peaks[2]); Serial.print(" | ");
    Serial.print(f_peaks[3]); Serial.print(" | ");
    ...
    for (int i = 0; i < 12; i++) {
      in[i] = 0; // utilising in[] array for further calculation
    }
